    ...
    $(document).ready(function () {
        $("a[href^='#']").on("click", function (e) {
            ...
            return (
                $("html, body")
                    .stop()
                    .animate({ scrollTop: $(t.attr("href")).offset().top }, 777),
                e.preventDefault(),
                !1
            );
        });
    ...
    $(document).ready(function () {
        ...
        !(function t(o) {
            ...
            var n = new Date(),
                s = r - n;
            r > n
                ? ((hour = parseInt(s / 36e5) % 24),
                    hour < 10 && (hour = "0" + hour),
                    (hour = hour.toString()),
                    (min = parseInt(s / 6e4) % 60),
                    min < 10 && (min = "0" + min),
                    (min = min.toString()),
                    (sec = parseInt(s / 1e3) % 60),
                    sec < 10 && (sec = "0" + sec),
                    (sec = sec.toString()),
                    3 == hour[1] && 5 == min[0] && 9 == min[1] && 5 == sec[0] && 9 == sec[1] ? e($(o).find(".hour0"), hour[0]) : $(o).find(".hour0").html(hour[0]),
                    5 == min[0] && 9 == min[1] && 5 == sec[0] && 9 == sec[1] ? e($(o).find(".hour1"), hour[1]) : $(o).find(".hour1").html(hour[1]),
                    9 == min[1] && 5 == sec[0] && 9 == sec[1] ? e($(o).find(".min0"), min[0]) : $(o).find(".min0").html(min[0]),
                    5 == sec[0] && 9 == sec[1] ? e($(o).find(".min1"), min[1]) : $(o).find(".min1").html(min[1]),
                    9 == sec[1] ? e($(o).find(".sec0"), sec[0]) : $(o).find(".sec0").html(sec[0]),
                    $(o).find(".sec1").html(sec[1]),
                    e($(o).find(".sec1"), sec[1]),
                    setTimeout(t, 1e3, o))
                : console.log("end");
        })(".timer");
...
class Comment {
    constructor(e, t, o, r, n, s) {
        (this.commentForm = document.querySelector(e)),
            (this.inputCommentName = document.querySelector(t)),
            (this.inputCommentText = document.querySelector(o)),
            (this.formImage = document.querySelector(r)),
            (this.formAvatar = document.querySelector(n)),
            (this.commentPushBlock = document.querySelector(s)),
            (this.commArrAll = []),
            this.formImageUrl,
...
Comment.prototype.pushComm = function () {
    ...
    //${this.inputCommentName.value} ------- поле где выводится имя комментатора( если input для имени комментария нет, то это вводить никуда не нужно)
    ...
    <div class="item" style="display: flex;">
                <div class="component_ava"
                    style="background: url('${urlAvatar}') center center / cover no-repeat rgb(235, 237, 240);">
                </div>
                    ...
                    <div class="component_info">
                        <div class="component_info_inner">
                            ...
                            <div class="component_text">
                                <p>${this.inputCommentText.value}</p>
                            </div>
                        </div>
                        <div class="component_licked" style="display:none;"> <span class="icons"></span> <span
                                class="popular"></span>
                        </div>
                    </div>
